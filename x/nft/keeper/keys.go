...
func classOwner(classID string) []byte {
	key := make([]byte, len(ClassKey)+len(OwnerKey)+len(classID))
	copy(key, ClassKey)
	copy(key[len(ClassKey):], OwnerKey)
	copy(key[len(ClassKey)+len(OwnerKey):], classID)
	return key
}
...
func nftOfClassByOwnerStoreKey(owner sdk.AccAddress, classID string) []byte {
	...
	var key = make([]byte, len(NFTOfClassByOwnerKey)+len(owner)+len(classIDBz))
...
func ownerStoreKey(classID, nftID string) []byte {
	...
	var key = make([]byte, len(OwnerKey)+len(classIDBz)+len(Delimiter))
	copy(key, OwnerKey)
