...
func (k Keeper) SaveClass(ctx sdk.Context, class nft.Class) error {
	if err := nft.ValidateClassID(class.Id); err != nil {
		return err
	}
...
func (k Keeper) GetClassOwner(ctx sdk.Context, classID string) sdk.AccAddress {
	store := ctx.KVStore(k.storeKey)
	bz := store.Get(classOwner(classID))
	return sdk.AccAddress(bz)
}

func (k Keeper) SetClassOwner(ctx sdk.Context, classID string, owner sdk.AccAddress) {
	store := ctx.KVStore(k.storeKey)
	store.Set(classOwner(classID), owner.Bytes())
}
